...
parser.add_argument('--download_model_dir', default=None, type=str, help='Local model dir')
...
def speaker_confirm_main(model_id=None, wavs=None, pre_model=None):
    """
    说话人确认函数
    :param model_id: 模型id: str
    :param wavs: 确认的wav文件: list
    :param pre_model: 预训练模型权重地址: str
    :return: 相关值: float
    """
    ...
    if args.pre_model is None:
        ...
        save_dir = os.path.join(args.download_model_dir, args.model_id.split('/')[1])
    ...
    def compute_embedding(wav_file, save=True):
        if type(wav_file) is str:
            ...
            # 转成tensor 因为speaker_confirm 数据源是tensor 维度是（1，xxx）
        ...
        if save:
            if args.pre_model is None:
                if type(wav_file) is str:  # 原始地址保存

                    save_path = embedding_dir / (
                            '%s.npy' % (os.path.basename(wav_file).rsplit('.', 1)[0]))
                else:
                    save_path = embedding_dir + "/" + "speaker_confirm.npy"
            else:
                if type(wav_file) is str:  # 原始地址保存
...
if __name__ == '__main__':
    ...
    # wavs = [r"D:\asr_demo\dataset\data\data\data_thchs30\test\D4_750.wav",
    #         r"D:\asr_demo\dataset\data\data\data_thchs30\test\D4_751.wav"]  # 长度是0或者2
    # pre_model = r"D:\shuohuaren\3D_Speaker\model\speech_eres2net_sv_zh-cn_16k-common\pretrained_eres2net_aug.ckpt"
    # # args.pre_model = None
    # speaker_confirm_main(model_id, wavs, pre_model)
